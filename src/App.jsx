...
function AdminOnly({ user, profile, children }) {
...
export default function App() {
  ...
  const addToCart = (product) => {
    ...
    setCart((old) => {
      ...
      if (found) return old.map((i) => i.id === product.id ? { ...i, qty: i.qty + 1 } : i);
      return [...old, { id: product.id, name: product.name, price: Number(product.price), unit: product.unit || 'un', imageUrl: product.imageUrl || '', qty: 1 }];
    });
  };
  ...
  return (
    <AppContext.Provider value={{ user, profile, cart, setCart, addToCart, cartCount }}>
      <Header />
      <Routes>
