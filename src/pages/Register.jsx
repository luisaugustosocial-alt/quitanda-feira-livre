...
export default function Register(){
 ...
 const findSuggestions=async(base)=>{
   ...
   for(let i=1;i<=99 && found.length<3;i++){
     ...
     if(candidate && !(await usernameExists(candidate))) found.push(candidate);
   }
   return found;
 };
 ...
 const checkUsername=async()=>{
   ...
   try{
     ...
     if(exists){
       ...
       setSuggestions(await findSuggestions(candidate));
       return true;
     }
     if(msg==='Este usuário já está em uso.') setMsg('');
     return false;
 ...
 const submit=async(e)=>{e.preventDefault();setMsg('');setSuggestions([]);let created=null;try{
 ...
 <label>Escolha seu nome de usuário<div className="username-field"><span>userquitanda@</span><input required value={form.username} onChange={e=>{setForm({...form,username:e.target.value});setSuggestions([]);if(msg==='Este usuário já está em uso.')setMsg('')}} onBlur={checkUsername}/></div>{checking&&<small>Verificando disponibilidade...</small>}</label>
